-- ─────────────────────────────────────────────────────────────────────
-- Lingora — 0000_init.sql
-- Canonical linearised schema. Source: docs/07-DATABASE-SCHEMA.md.
-- A single SQL file applied once (NOT 45 competing patches). RLS is enabled
-- on EVERY table. Conventions: schema public; PKs UUID DEFAULT gen_random_uuid();
-- timestamps timestamptz DEFAULT now(); the Supabase JS client (anon + user
-- JWT) is the only renderer client; service-role key lives only in Edge Fns.
--
-- Order (§14) tightened so FK referenced tables exist before referencing ones:
-- extensions → helpers → profiles/roles → courses → progress → sessions →
-- conversation → vocab/achievements/leaderboards → terms/lessons/materials →
-- books/highlights → grammar → assessment_sessions → cefr_assessment_questions →
-- assessment_tasks → assessments → proficiency → criteria/text_simplifications →
-- modules/paths/rules/engagement → enrollments/reviews/wishlist/certs/reminders/
-- lesson-progress/submissions → assignments/questions → notifications/templates/
-- analytics/media/events/ai → settings → live conversation → wizard helpers →
-- triggers → storage bucket → seed.
-- ─────────────────────────────────────────────────────────────────────
...
-- Shared trigger fn: stamps updated_at on every update of every table that
-- has one. Usage: `create trigger <t>_updated_at before update on <table>
--   for each row execute function public.set_updated_at();`
create or replace function public.set_updated_at() returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end; $$;

-- Auth/role helpers — `user_roles` is the ONLY admin source (no email-domain,
-- no JWT-claim inspection). See 07 §0.
...
-- Auto-provision on signup (SECURITY DEFINER trigger on auth.users).
...
create policy "admin view roles"   on public.user_roles for select to authenticated using (public.is_admin());
create policy "super_admin assigns roles" on public.user_roles for insert to authenticated
  with check (public.is_super_admin());
create policy "super_admin updates roles" on public.user_roles for update to authenticated
  using (public.is_super_admin()) with check (public.is_super_admin());
revoke all on public.user_roles from anon;

-- 2. Courses (declared before tables that FK to it) ─────────────────────
create table public.courses (
  ...
  review_html        text,
  is_active          boolean default false,                -- draft until author publishes
...
create policy "view active courses" on public.courses for select using (is_active or public.is_instructor());
create policy "instructor manage own" on public.courses for all to authenticated
...
create policy "own progress r" on public.user_progress for select using (auth.uid() = user_id);
create policy "own progress w" on public.user_progress for insert with check (auth.uid() = user_id);
create policy "own progress u" on public.user_progress for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view progress" on public.user_progress for select to authenticated using (public.is_admin());
...
create policy "own sessions r" on public.learning_sessions for select using (auth.uid() = user_id);
create policy "own sessions w" on public.learning_sessions for insert with check (auth.uid() = user_id);
create policy "own sessions u" on public.learning_sessions for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view sessions" on public.learning_sessions for select to authenticated using (public.is_admin());

-- conversation_history — the canonical chat store (replaces baseline's non-existent chat_messages).
...
create policy "own conv r" on public.conversation_history for select using (auth.uid() = user_id);
create policy "own conv w" on public.conversation_history for insert with check (auth.uid() = user_id);
create policy "own conv u" on public.conversation_history for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create index idx_uservocab_due on public.user_vocabulary(user_id, last_reviewed_at);
create trigger uservocab_updated_at before update on public.user_vocabulary
  for each row execute function public.set_updated_at();
alter table public.user_vocabulary enable row level security;
create policy "own vocab all" on public.user_vocabulary for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view vocab" on public.user_vocabulary for select to authenticated using (public.is_admin());
...
create policy "view active badges" on public.badges for select using (is_active);
...
create policy "own badges r" on public.user_badges for select using (auth.uid() = user_id);
create policy "system awards badge" on public.user_badges for insert with check (true);   -- Edge Fn (service role)
create policy "system updates badge" on public.user_badges for update using (true) with check (true);
...
create policy "own ach r" on public.user_achievements for select using (auth.uid() = user_id);
create policy "own ach w" on public.user_achievements for insert with check (auth.uid() = user_id);

-- Leaderboards + entries (opt-in).
...
create policy "view active lb" on public.leaderboards for select using (is_active);
alter table public.leaderboard_entries enable row level security;
create policy "view lb entries" on public.leaderboard_entries for select using (true);
create policy "own write entry" on public.leaderboard_entries for insert with check (auth.uid() = user_id);
create policy "own update entry" on public.leaderboard_entries for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- 4. Terms → lessons → materials ────────────────────────────────────────
...
create policy "view terms" on public.terms for select using (true);
create policy "instructor manage terms" on public.terms for all to authenticated
...
-- lessons keeps the seven REQUIRED columns its getter selects; the baseline
-- migration dropped them and broke its own query.
...
create policy "view published lessons" on public.lessons for select using (is_published or public.is_instructor());
create policy "instructor manage lessons" on public.lessons for all to authenticated
...
create policy "view materials" on public.lesson_materials for select using (true);
create policy "instructor manage materials" on public.lesson_materials for all to authenticated
...
-- Books + PDF highlights → vocabulary.
...
create policy "view books" on public.books for select using (true);
create policy "instructor manage books" on public.books for all to authenticated
...
create policy "own highlights" on public.word_highlights for select using (auth.uid() = user_id);
create policy "own highlights w" on public.word_highlights for insert with check (auth.uid() = user_id);
create policy "own highlights u" on public.word_highlights for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "own highlights d" on public.word_highlights for delete using (auth.uid() = user_id);
...
create policy "view active grammar" on public.grammar_lessons for select using (is_active);
create policy "instructor manage grammar" on public.grammar_lessons for all to authenticated
  using (public.is_instructor()) with check (public.is_instructor());

-- 5. Assessment & CEFR ───────────────────────────────────────────────────
-- assessment_sessions first; the trigger below upserts proficiency +
-- updates the profile when status->completed.
...
create policy "own assess r" on public.assessment_sessions for select using (auth.uid() = user_id);
create policy "own assess w" on public.assessment_sessions for insert with check (auth.uid() = user_id);
create policy "own assess u" on public.assessment_sessions for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view assess" on public.assessment_sessions for select to authenticated using (public.is_admin());
...
create policy "view active questions" on public.cefr_assessment_questions for select using (is_active);
create policy "instructor manage questions" on public.cefr_assessment_questions for all to authenticated
  using (public.is_instructor()) with check (public.is_instructor());

-- assessment_tasks (FK to questions now satisfiable).
...
create policy "own tasks r" on public.assessment_tasks for select using (
  exists (select 1 from public.assessment_sessions s where s.id = session_id and s.user_id = auth.uid()));
create policy "own tasks w" on public.assessment_tasks for insert with check (
  exists (select 1 from public.assessment_sessions s where s.id = session_id and s.user_id = auth.uid()));
create policy "own tasks u" on public.assessment_tasks for update using (
  exists (select 1 from public.assessment_sessions s where s.id = session_id and s.user_id = auth.uid()))
  with check (exists (select 1 from public.assessment_sessions s where s.id = session_id and s.user_id = auth.uid()));

-- per-lesson assessments (separate concept).
...
create policy "own prof r" on public.user_proficiency_profiles for select using (auth.uid() = user_id);
create policy "own prof w" on public.user_proficiency_profiles for insert with check (auth.uid() = user_id);
create policy "own prof u" on public.user_proficiency_profiles for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- Trigger: when an assessment session transitions to 'completed', upsert
-- proficiency + the user's CEFR level + assessment_completed flag.
...
-- assessment_criteria + text_simplifications cache.
...
create policy "view criteria" on public.assessment_criteria for select using (true);
...
create policy "view simplifications" on public.text_simplifications for select using (true);
create policy "instructor create simplif" on public.text_simplifications for insert to authenticated with check (public.is_instructor());

-- 6. Progression & engagement ──────────────────────────────────────────
...
create policy "view active modules" on public.content_modules for select using (is_active);
create policy "instructor manage modules" on public.content_modules for all to authenticated
  using (public.is_instructor()) with check (public.is_instructor());
...
create policy "own modprogress" on public.user_module_progress for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create trigger lpaths_updated_at before update on public.learning_paths
  for each row execute function public.set_updated_at();
alter table public.learning_paths enable row level security;
create policy "view active paths" on public.learning_paths for select using (is_active);
create policy "instructor manage paths" on public.learning_paths for all to authenticated using (public.is_instructor()) with check (public.is_instructor());
...
create trigger ulpaths_updated_at before update on public.user_learning_paths
  for each row execute function public.set_updated_at();
alter table public.user_learning_paths enable row level security;
create policy "own lpath" on public.user_learning_paths for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "view active rules" on public.progression_rules for select using (is_active);
create policy "instructor manage rules" on public.progression_rules for all to authenticated using (public.is_instructor()) with check (public.is_instructor());
...
create policy "own engage r" on public.conversation_engagement for select using (auth.uid() = user_id);
create policy "own engage w" on public.conversation_engagement for insert with check (auth.uid() = user_id);
create policy "own engage u" on public.conversation_engagement for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- 7. Enrollments, reviews, wishlist, certs, reminders, lesson-progress, submissions ──
...
create policy "own enroll r" on public.course_enrollments for select using (auth.uid() = user_id);
create policy "own enroll w" on public.course_enrollments for insert with check (auth.uid() = user_id);
create policy "own enroll u" on public.course_enrollments for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "instructor view enroll" on public.course_enrollments for select to authenticated
...
create trigger ulp_updated_at before update on public.user_lesson_progress
  for each row execute function public.set_updated_at();
alter table public.user_lesson_progress enable row level security;
create policy "own ulp" on public.user_lesson_progress for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "own sub r" on public.lesson_submissions for select using (auth.uid() = user_id);
create policy "own sub w" on public.lesson_submissions for insert with check (auth.uid() = user_id);
create policy "own sub u" on public.lesson_submissions for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "view approved reviews" on public.course_reviews for select using (is_approved or auth.uid() = user_id);
create policy "own review w" on public.course_reviews for insert with check (auth.uid() = user_id);
create policy "own review u" on public.course_reviews for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin approve reviews" on public.course_reviews for update to authenticated using (public.is_admin()) with check (public.is_admin());
...
create policy "own wishlist" on public.user_course_wishlist for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "view active certs" on public.certificates for select using (is_active);
create policy "admin manage certs" on public.certificates for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
create policy "own ucert r" on public.user_certificates for select using (auth.uid() = user_id);
create policy "system issue ucert" on public.user_certificates for insert with check (true);  -- service role
create policy "own ucert u" on public.user_certificates for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create trigger reminders_updated_at before update on public.study_reminders
  for each row execute function public.set_updated_at();
alter table public.study_reminders enable row level security;
create policy "own reminders" on public.study_reminders for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- 8. Assignments & questions ────────────────────────────────────────────
...
create policy "view active assignments" on public.assignments for select using (is_active);
create policy "instructor manage assignments" on public.assignments for all to authenticated
...
create policy "view questions" on public.questions for select using (true);
create policy "instructor manage questions" on public.questions for all to authenticated
...
-- 9. Notifications, templates, analytics, media, AI ─────────────────────
-- EXACTLY ONE notification table.
...
create policy "own notif r" on public.notifications for select using (auth.uid() = user_id);
create policy "own notif u" on public.notifications for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view notif" on public.notifications for select to authenticated using (public.is_admin());
...
create policy "view active templates" on public.notification_templates for select to authenticated using (is_active and public.is_admin());
...
create trigger course_templates_updated_at before update on public.course_templates
  for each row execute function public.set_updated_at();
alter table public.course_templates enable row level security;
create policy "view active ct" on public.course_templates for select to authenticated using (is_active and public.is_instructor());
create policy "manage ct" on public.course_templates for all to authenticated using (public.is_instructor()) with check (public.is_instructor());
...
create policy "own analytics r" on public.user_analytics for select using (auth.uid() = user_id);
create policy "admin view analytics" on public.user_analytics for select to authenticated using (public.is_admin());
...
create trigger course_analytics_updated_at before update on public.course_analytics
  for each row execute function public.set_updated_at();
alter table public.course_analytics enable row level security;
create policy "admin view course analytics" on public.course_analytics for select to authenticated using (public.is_admin());
...
create policy "own events w" on public.learning_events for insert with check (auth.uid() = user_id);
create policy "own events r" on public.learning_events for select using (auth.uid() = user_id);
create policy "admin view events" on public.learning_events for select to authenticated using (public.is_admin());
...
create policy "admin view ai gen" on public.ai_content_generation for select to authenticated using (public.is_admin());
...
create policy "own recs r" on public.ai_recommendations for select using (auth.uid() = user_id);
create policy "own recs u" on public.ai_recommendations for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "view media" on public.media_content for select using (true);
create policy "instructor manage media" on public.media_content for all to authenticated
  using (exists (select 1 from public.lessons l
            join public.courses c on c.id = l.course_id
            where l.id = media_content.lesson_id and (c.instructor_id = auth.uid() or public.is_admin())))
  with check (exists (select 1 from public.lessons l
            join public.courses c on c.id = l.course_id
            where l.id = media_content.lesson_id and (c.instructor_id = auth.uid() or public.is_admin())));

-- 10. System settings (EXACTLY ONE definition — JSONB value version) ────
...
create policy "view public settings" on public.system_settings for select using (is_public);
create policy "admin manage settings" on public.system_settings for all to authenticated
  using (public.is_admin()) with check (public.is_admin());

-- 11. Live conversation ─────────────────────────────────────────────────
...
create policy "own live r" on public.live_conversation_sessions for select using (auth.uid() = user_id);
create policy "own live w" on public.live_conversation_sessions for insert with check (auth.uid() = user_id);
create policy "own live u" on public.live_conversation_sessions for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "admin view live" on public.live_conversation_sessions for select to authenticated using (public.is_admin());
alter table public.live_conversation_messages enable row level security;
create policy "own live msg r" on public.live_conversation_messages for select using (
...
create policy "own live msg w" on public.live_conversation_messages for insert with check (
  exists (select 1 from public.live_conversation_sessions s
          where s.session_id = live_conversation_messages.session_id and s.user_id = auth.uid()));

-- 12. Course-wizard helper functions ────────────────────────────────────
...
begin
  for v in select * from jsonb_array_elements(p_lessons) loop
    ...
    if v ? 'materials' then
      insert into public.lesson_materials (lesson_id, type, url, content, metadata, order_number)
      select v_id, m->>'type', m->>'url', m->>'content', m->>'metadata', (m->>'order_number')::int
      from jsonb_array_elements(v->'materials') m;
    end if;
    cnt := (select count(*) from public.lesson_materials where lesson_id = v_id);
...
-- Realtime publication for the notifications table (Phase 6 broadcast push).
do $$ begin
...
create policy "Auth upload course-assets" on storage.objects for insert to authenticated
  with check (bucket_id='course-assets' and (auth.uid() is not null));
...
-- avatars: private, per-user path scope.
create policy "Owner read avatar" on storage.objects for select to authenticated
...
create policy "Owner delete avatar" on storage.objects for delete to authenticated
  using (bucket_id='avatars' and owner=auth.uid());

-- ──────────────────────────────────────────────────────────────────────
-- End of 0000_init.sql. Dev seeds: see supabase/seed/seed.sql
-- (CEFR question samples, badge defs, assessment criteria, sample course).
